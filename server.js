...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/api/agents') {
    setSecurityHeaders(res);
    try {
      const kanbanFile = path.join(WORKSPACE_DIR, 'kanban.json');
      ...
      // Read kanban tasks
      try {
        const kanbanData = JSON.parse(fs.readFileSync(kanbanFile, 'utf-8'));
        kanbanTasks = kanbanData.tasks || [];
      } catch(e) {}

      // System agent names to filter out
      const systemAgents = ['main', 'orchestrator', 'claude'];

      // All possible company employees (shown even without tasks)
      const allEmployees = [
        { name: 'Senior Java Developer', avatar: '👨‍💻', type: 'senior' },
        { name: 'Junior Java Developer', avatar: '🧑‍💻', type: 'junior' },
        { name: 'QA Engineer', avatar: '🧪', type: 'junior' },
        { name: 'Senior Security Analyst', avatar: '🕵️', type: 'senior' },
        { name: 'Senior DevOps Engineer', avatar: '🔧', type: 'senior' },
        { name: 'Junior DevOps Engineer', avatar: '🛠️', type: 'junior' },
        { name: 'Frontend Developer', avatar: '🎨', type: 'junior' },
        { name: 'Technical Writer', avatar: '📝', type: 'junior' }
      ];

      // Create agents from kanban tasks with assignees
      const activeAssignees = new Set();
      for (const task of kanbanTasks) {
        if (!task.assignee) continue;
        activeAssignees.add(task.assignee);

        // Determine status based on column
        let status = 'idle';
        if (task.column === 'in-progress') status = 'working';
        else if (task.column === 'review') status = 'review';
        else if (task.column === 'backlog' || task.column === 'done') status = 'idle';

        // Determine type from assignee name
        const assigneeLower = task.assignee.toLowerCase();
        let type = 'junior';
        if (assigneeLower.includes('senior')) type = 'senior';

        const emp = allEmployees.find(e => e.name === task.assignee);
      ...
      // Add employees without tasks (idle in break room)
      for (const emp of allEmployees) {
        if (!activeAssignees.has(emp.name)) {
          agents.push({
      ...
      // Also include truly active subagents from OpenClaw (but NOT system agents)
      if (fs.existsSync(agentsDir)) {
        ...
        for (const agentId of agentDirs) {
          // Skip system agents
          if (systemAgents.includes(agentId.toLowerCase())) continue;

          const sessPath = path.join(agentsDir, agentId, 'sessions');
          if (!fs.existsSync(sessPath)) continue;
          try {
            const sessions = fs.readdirSync(sessPath).filter(f => f.endsWith('.json'));
            for (const sessFile of sessions) {
              try {
                const sess = JSON.parse(fs.readFileSync(path.join(sessPath, sessFile), 'utf-8'));
                ...
                agents.push({
                  id: agentId,
                  name: sess.label || agentId,
                  status: 'working',
                  currentTask: lastMsg ? lastMsg.content.substring(0, 80) : '',
                  project: 'OpenClaw',
                  type: 'subagent',
                  startedAt: sess.createdAt || null,
                  lastActivity: lastMsg ? lastMsg.timestamp || Date.now() : Date.now()
                });
              } catch(e2) {}
            }
          } catch(e1) {}
      ...
      // Deduplicate by name, prefer working status
      const seen = {};
      const unique = [];
      for (const a of agents) {
        const key = a.name;
        if (!seen[key]) { seen[key] = a; unique.push(a); }
        else if (a.status === 'working' && seen[key].status !== 'working') {
          const idx = unique.findIndex(u => u.name === key);
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ agents: limited }));
    } catch(e) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.url === '/api/agents/spawn' && req.method === 'POST') {
    setSecurityHeaders(res);
    let body = '';
    ...
    req.on('end', () => {
      try {
        ...
        const spawnFile = path.join(dataDir, 'spawned-agents.json');
        let spawned = [];
        try { spawned = JSON.parse(fs.readFileSync(spawnFile, 'utf-8')); } catch(e) {}
        const agent = { id: 'agent-' + Date.now(), name: data.label || 'Agent', task: data.task, status: 'working', startedAt: new Date().toISOString() };
        spawned.push(agent);
        fs.writeFileSync(spawnFile, JSON.stringify(spawned, null, 2));
        res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.url === '/api/agents/kill' && req.method === 'POST') {
    setSecurityHeaders(res);
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
      try {
        ...
        let spawned = [];
        try { spawned = JSON.parse(fs.readFileSync(spawnFile, 'utf-8')); } catch(e) {}
        spawned = spawned.filter(a => a.id !== data.id);
        fs.writeFileSync(spawnFile, JSON.stringify(spawned, null, 2));
        res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.url.startsWith('/api/agents/') && req.url.endsWith('/logs')) {
    setSecurityHeaders(res);
    const agentId = decodeURIComponent(req.url.replace('/api/agents/', '').replace(/\/logs$/, ''));
    try {
      const sessPath = path.join(OPENCLAW_DIR, 'agents', agentId, 'sessions');
      if (!fs.existsSync(sessPath)) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ messages: [] }));
        return;
      }
      const sessions = fs.readdirSync(sessPath).filter(f => f.endsWith('.json')).sort();
      ...
      const sess = JSON.parse(fs.readFileSync(path.join(sessPath, latest), 'utf-8'));
      const messages = (sess.messages || []).slice(-50).map(m => ({
        role: m.role,
